...
# --- VARIABLES POUR LE CERVEAU DE L'IA (LA FEINTE) ---
enum State { PATROL, STARE, FAKE_LEAVE, RUSH_BACK, STARE_AGAIN }
...
func _process(delta):
	...
	match current_state:
		...
		State.RUSH_BACK:
			...
			state_timer += delta
			# CORRECTION ICI : On laisse 1.5s (au lieu de 0.8) pour être sûr qu'il finisse son sprint
...
func check_player_cheating():
	if player and not player.game_is_over:
		...
		# NOUVEAU : Si le prof est en pleine feinte, il a les yeux rivés sur toi !
		var le_prof_me_fixe = current_state in [State.STARE, State.RUSH_BACK, State.STARE_AGAIN]
		
		# Si tu triches ET (qu'il est derrière toi OU qu'il te dévisage de près)
		if player.is_cheating and (est_derriere_moi or le_prof_me_fixe):
			print("CRAMÉ ! Le prof t'a vu !")
			player.game_over()
